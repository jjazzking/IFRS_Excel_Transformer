...
| **의사록 읽기** (`#/minutes`) | 의사록 원본 PDF | 뽑아낸 정규 스키마 + 검토 필요 표시 | 사용 중 |
...
왼쪽에서 통화와 기간을 고르면 일자별 고시 환율이 나오고, 담은 날짜가 오른쪽 엑셀
미리보기에 조서 모양 그대로 섭니다.

- **기간 평균환율·기말환율**을 계산해 행으로 붙입니다. 손으로 하던 계산입니다.
...
이사회 의사록 PDF 를 **정규 데이터 스키마** 로 옮깁니다. 뽑은 값마다 그 값이 원문
어디에서 나왔는지 함께 보여주고, 확신이 없는 자리는 값을 지어내지 않고 `검토 필요` 로
남깁니다.

### 브라우저에서 — `#/minutes`

의사록 PDF 를 끌어다 놓으면 왼쪽에 원본, 오른쪽에 뽑아낸 스키마가 나옵니다. 오른쪽 칸을
**아무 데나 누르면** 왼쪽 원문의 그 자리가 칠해지면서 **화면 가운데로** 옵니다 — 쪽을
찾아 내려갈 일이 없습니다. 의안 카드 안의 제목 · 발췌 · 금액은 더 좁은 자리를 가리킵니다.

> **파일이 브라우저 밖으로 나가지 않습니다.** 규칙만 쓰므로 서버도, 키도, 네트워크
> 전송도 없습니다. 이사회 의사록은 미공개 중요정보라 이 성질이 기능 하나보다 중요합니다.

읽는 것은 **일시 · 장소 · 출석 인원 · 의안**입니다. 의안마다 제목 · 가결 여부 · 표결 수 ·
결의 문장 발췌를 뽑고, 금액과 **펴 볼 기준서 후보**까지 붙입니다. 의안 카드에서 `의안 원문
전체 보기` 를 펴면 잘라낸 **본문 전체**를 원문 그대로 볼 수 있습니다 — 이 저장소에 기준서
문단 3,661건이 이미 있어서 가능한 일입니다. 영향 여부는 판단하지 않습니다. 규칙은 어디를
볼지까지만 좁히고, 판단은 사람이 합니다.

스캔 페이지는 **브라우저 안에서 OCR 로 읽습니다.** 엔진과 한국어 모델까지 전부 이 사이트에서
받으므로 여전히 바깥으로 나가는 것이 없습니다. 스캔 페이지를 처음 만났을 때만 8.4MB 를 받고
그다음부터는 브라우저가 쟁여 둡니다 — 텍스트 의사록만 여는 분에게는 한 바이트도 나가지
않습니다. OCR 로 읽은 값에는 `OCR` 표가 붙습니다. 그 글자는 원문 그대로가 아니니까요.

두 판이 같은 답을 내는지는 PDF 표본 4건과 **무작위로 서식을 섞은 본문 6,000건**으로
견줬습니다. 본문 문자열이 글자 단위로 같고, 모든 필드와 근거 구간에서 불일치가 0건입니다
([`docs/minutes-plan.md`](docs/minutes-plan.md) 7장).

### 스크립트로 — 정확도 재기

화면보다 스크립트를 먼저 만들었습니다. 정확도를 먼저 재기 위해서입니다.
...
| 스캔 열화본 70건 — 규칙 + OCR | **71.7%** (642/896) |
...
| --- | ---: |
| 저해상도 · 스마트폰 촬영 | 85~86% |
| 사무실 스캔 · 열화 없음 · 복사기 사본 | 78~81% |
| 오래된 종이 | 74.6% |
| **팩스 (흑백 2치화)** | **24.3%** |

70% 에서 막힌 것은 규칙이 아니라 OCR 입니다. 글자를 거의 맞게 읽어도 `…의 건` 이
`…의 견` 이 되면 그 자리는 0점입니다. 정규식으로는 못 고칩니다. 팩스는 원본을 다시
받는 편이 빠릅니다.

### OCR 엔진을 비교했습니다

규칙을 빼고 글자만 비교했습니다. 원문 대비 문자 일치율입니다.

| 엔진 · 설정 | 글자 일치 | 초/건 |
| --- | ---: | ---: |
| Tesseract psm 6 · kor — **처음 쓰던 설정** | 66.7% | 11.5 |
| **Tesseract psm 4 · kor+eng** | **80.4%** | 4.8 |
| EasyOCR ko+en | 72.5% | 22.9 |

**엔진을 바꾸는 것보다 설정을 바꾸는 것이 컸고, 공짜였습니다.** 쪽을 어떻게 나눠 볼지를
정하는 값 하나입니다. 복사기 사본이 53%에서 95%로, 오래된 종이가 52%에서 91%로
올랐습니다. 이진화·선명화·확대 같은 전처리는 전부 손해였습니다.

클라우드 엔진(Google Vision, CLOVA, Upstage)은 키가 없고 파일을 밖으로 보내는 일이라
보안 심의가 먼저입니다. 자세한 비교는 [`docs/minutes-ocr.md`](docs/minutes-ocr.md) 4-4 에
있습니다.
