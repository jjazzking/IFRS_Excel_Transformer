...
| **의사록 읽기** (`#/minutes`) | 의사록 원본 PDF | 뽑아낸 정규 스키마 + 검토 필요 표시 | 사용 중 — 문서 레벨까지 (의안은 아직) |
...
지금 읽는 범위는 **일시 · 장소 · 출석 인원까지**입니다. 의안 규칙은 아직 아래 파이썬
판에만 있어서, 화면의 의안 칸은 비어 있고 그렇다고 말해 둡니다 — 못 찾은 것과 아직 안
본 것은 다릅니다.

두 판이 같은 답을 내는지는 합성 표본 4건으로 견줬습니다. 본문 문자열이 4/4 건 완전
일치하고, 필드값 28/28 · 근거 구간 16/16 이 같습니다
