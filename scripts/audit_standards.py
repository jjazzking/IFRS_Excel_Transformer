#!/usr/bin/env python3
"""기준서 DB footing(정합성) 1차 자동 검증.

사람이 40개 기준서 3,661개 문단을 전수로 읽는 대신, 기계가 잡을 수 있는 결함을
먼저 걸러 검수자에게 '확인이 필요한 지점'만 넘긴다. 판정은 사람이 한다 —
이 스크립트는 후보를 좁힐 뿐이다.

    python3 scripts/audit_standards.py                     # 요약만
    python3 scripts/audit_standards.py --csv footing.csv   # 검수 워크시트 생성
...
# 심각도: P1 배포 전 반드시 해소 / P2 표본 확인 후 판단 / P3 참고
SEVERITY = {
    'NUM_GAP': 'P1',
    'DUP_BODY': 'P1',
    'TOO_SHORT': 'P1',
...
def main() -> int:
    ...
    ap.add_argument('--csv', help='검수 워크시트(CSV) 저장 경로')
    ap.add_argument('--json', dest='json_out', help='검출 결과 JSON 저장 경로')
    ap.add_argument('--assign', type=int, metavar='N', help='검수자 N명에게 기준서 배분')
    ap.add_argument('--severity', default='P3', choices=['P1', 'P2', 'P3'],
                    help='이 심각도까지 출력 (기본 P3 = 전부)')
    args = ap.parse_args()
    ...
    total_p = sum(para_count.values())
    print(f'기준서 {len(standards)}건 / 문단 {total_p:,}건 검사\n')
    ...
    touched = len({(f['standardId'], f['paragraph']) for f in all_findings
                   if f['severity'] in ('P1', 'P2')})
    print(f'합계 {len(all_findings):,}건 (P1 {p1:,} · P2 {p2:,})')
    print(f'→ 사람이 눈으로 확인할 지점: {touched:,}곳 (전체 문단의 {touched / total_p:.1%})')

    print('\n기준서별 P1/P2 상위')
    ranked = sorted(per_std.items(),
                    key=lambda kv: -sum(1 for f in kv[1] if f['severity'] in ('P1', 'P2')))
    for sid, found in ranked[:10]:
        a = sum(1 for f in found if f['severity'] == 'P1')
        b = sum(1 for f in found if f['severity'] == 'P2')
        if a + b:
            print(f'  {sid:<16} 문단 {para_count[sid]:>4}  P1 {a:>3}  P2 {b:>3}')

    if args.assign:
        n = args.assign
        print(f'\n검수자 {n}명 배분 (기준서 수를 고르게 나누되, 확인지점이 많은 기준서를 번갈아 배정)')
        buckets = [[] for _ in range(n)]
        # 확인지점이 많은 순으로 뱀 모양(1→n, n→1) 배정: 담당 기준서 수와 부담을 함께 맞춘다.
        for idx, (sid, found) in enumerate(ranked):
            row = idx // n
            i = idx % n if row % 2 == 0 else n - 1 - (idx % n)
            buckets[i].append((sid, sum(1 for f in found if f['severity'] in ('P1', 'P2'))))
        for i, b in enumerate(buckets, 1):
            names = ', '.join(s.replace('k-ifrs-', '제') + '호' for s, _ in b)
            paras = sum(para_count[s] for s, _ in b)
            print(f'  검수자 {i}: 기준서 {len(b)}건 / 문단 {paras:,} / 확인지점 {sum(w for _, w in b):>3}곳'
                  f'\n    {names}')

    if args.csv:
        with open(args.csv, 'w', encoding='utf-8-sig', newline='') as fh:
            w = csv.writer(fh)
            w.writerow(['심각도', '기준서', '기준서명', '문단', '규칙', '검출근거',
                        '판정(정상/오류)', '수정내용', '검수자'])
            for f in sorted(all_findings, key=lambda f: (f['severity'], f['standardId'])):
                w.writerow([f['severity'], f['standardCode'], f['standardTitle'],
                            f['paragraph'], f['rule'], f['evidence'], '', '', ''])
        print(f"\n검수 워크시트: {args.csv} ({len(all_findings):,}행)")
