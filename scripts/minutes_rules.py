...
#
# **`_`·`*`·`.` 을 넣어 봤다가 뺐다.** 열화본에서 콜론이 그렇게 읽히는 일이 잦기는
# 하다 (`:`→`_` 81회, `.` 68회). 그런데 그 오독은 전부 **의안 본문의 조건 나열**
# (`적용이자율 : 연7.22%`) 에서 났고, 이 글자를 쓰는 세 자리는 모두 머리말 라벨 줄이다.
# 열화본 80건에서 넣기 전과 후가 한 자리도 다르지 않았다 (`docs/minutes-ocr.md` 4-10).
COLON = r"[:：ㆍ·∶;]"
