...
**태그를 밀면 릴리스가 생긴다.** 받는 사람이 GitHub 계정 없이 링크 하나로 받을 수
있는 길은 이것뿐이다 — Actions 의 artifact 는 로그인해야 보이고, 한 겹 더 압축되어
있고, 30일 뒤 지워진다.

```
git tag v0.1.0 && git push origin v0.1.0
  → Releases 에 기준서데스크-파이썬.zip 이 붙는다
```

급하면 `Actions → 데스크톱 꾸러미 → Run workflow` 로 artifact 만 받을 수도 있다.
다만 그 파일은 `standards-desk-python.zip` **안에** 우리 zip 이 들어 있는 모양이라,
동료에게 그대로 건네면 두 번 풀어야 한다.

릴리스 잡은 **파이썬 꾸러미에만 매달아 두었다.** exe 는 서명도 없고 손으로 켜 본 적이
없어서, 그쪽이 깨져도 정작 필요한 꾸러미는 나가야 하기 때문이다.
